import os
import os.path
from rawige.utils.db_versions import DB_VERSIONS
from rawige.utils.sqlite import Database
...
def _db_path():
    if platform == 'Windows':
        return os.path.join(os.environ['APPDATA'], 'rawige.db')
    elif platform == 'Linux':
        return os.path.expanduser('~/.config/rawige.db')
    else:
        return os.path.expanduser('~/rawige.db')


class RawigeDB(Database):
